...
- oci compliant image
- cncf app concept and architecture
- confident working with k8s resource definitions

performance based - real k8s tasks in real scenarios
- create scale deployments
- releaseing updates
- implement blue green deployments
15-20 tasks, 2 hour limit

time management is important

quick tasks - pod, image
complex tasks - ingress, routing

scoring is diffirent tasks
score is 66%

spend time on difficult questions
certificate valid for 2 years
to keep active - renew certificate

**Logistics of the exam**
Design - 20
- develop and configure contanirized application
	- proper images
	- workloads
	- multi ciontainer images
	- 
Deploymen t - 20
- deploy and update applications
- rolling updates, blue/green, canary deployments
- package management with helm & kustomize
observability & manintanenace - 15
- monitor troubleshoot and maintain application using 
	- health-check
	- probes
	- logs
	...
	- api deprecation
env & security =-25%
- configure application env & security
- config-maps, secrets and resource management
- service accounts and security contxts
- extending with CRDs and operators
- 
Services & networking
...
>[!important]
>
>Following list of imperative commands to generate yaml files quickly, format to follow is
> `kubectl create <resourcename> [options] --dry-run=client -o yaml > <filename>.yaml`
Create a directory called ckad snippets `mkdir ~/ckad-snippets`

>[!note] Generate YAML templates

1. Command  to generate a pod yaml file `kubectl run mypod --image=nginx --restart=Never --dry-run=client -o yaml > mypod.yaml`
2. Create a deployment yaml `kubectl create deployment mydeploy --image=nginx --replicas=2 --dry-run=client -o yaml > deployment.yaml`
3. Create a service and expose the ports: `kubectl expose deployment mydeploy --port=80 --target-port=80 --type=ClusterIP --dry-run=client -o yaml > service.yaml'
4. create a service and export set nodeport: `kubectl expose deployment mydeploy --port=80 --target-port=80 --type=NodePort --dry-run=client -o yaml > service-nodeport.yaml
5. Create a config-map: `kubectl create configmap myconfig --from-literal=APP_MODE=prod --from-literal=APP_COLOR=blue --dry-run=client -o yaml > configmap.yaml`
6. Create a secret: `kubectl create secret generic mysecret --from-literal=username=admin --from-literal=password=password123 --dry-run=client -o yaml > serviceaccount.yaml'
7. Create a service account: `kubectl create serviceaccount mysa --dry-run=client -o yaml > serviceaccount.yaml`
8. Create a role: `kubectl create role pod-reader --verb=get,list,watch --resources=pods --dry-run=client -o yaml > roles.yaml`
9. Create a rolebinding: `kubectl create rolebinding read-pods-binding --role=pod-reader --serviceaccount=default:mysa --dry-run=client -o yaml > rolebinding.yaml`
10. Create ingress rules: `kubectl create ingress myingress --rule="myapp.example.com/*=myservice:80" --dry-run=client -o yaml > ingress.yaml
11. Create a job: `kubectl create job myjob --image=busybox --dry-run=client -o yaml -- echo "Hello CKAD" > job.yaml'
12. Create a cronjob: `kubectl create cronjob mycron --image=busybox --schedule="*/5 * * * *" --dry-run=client -o yaml -- echo "Hello CKAD" > cronjob.yaml`
13. There is no imperative policy generator for Networks For network policies: use the following network concept policies page and download the yaml 

>[!important]
>while editifing the YAML in the fly and directly apply the kubernetes cluster use the redirection to vi 
>`kubectl run mypod --image=nginx --restart=Never --dry-run=client -o yaml | vi -`
> make the changes, save the file
> this will directly apply the editied yaml to the cluster 
...
set cul
set cuc
autocmd FileType yaml setlocal et ts=2 ai sw=2 nu sts=0 autoindent smartindent
```
...
Exam Scenario:
Set the context
kubectl config set-context --current --namespace=ps-dev
